...
class ProxyStorage(object):
    ...
    def __init__(
        self, local=None, remote=None, local_options=None, remote_options=None
    ):
        self.local_path = local or self.local
        self.local_options = local_options or self.local_options or {}
        self.local = self._load_backend(
        ...
        self.remote = self._load_backend(
    ...
    def _load_backend(self, backend=None, options=None, handler=LazyBackend):
        if backend is None:  # pragma: no cover
            raise ImproperlyConfigured(
                "The ProxyStorage class '%s' "
                "doesn't define a needed backend." % (self, backend)
            )
